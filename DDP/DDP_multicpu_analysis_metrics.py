...
class LaneDetectionModel(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.final_conv = nn.Conv2d(8, num_classes, kernel_size=1)  # Output segmentation mask
...
def train_ddp(rank, world_size, train_dataset, batch_size, lr, output_file, log_interval=120):
    ...
    training_start_time = datetime.now().isoformat()  # Log start time
    ...
    if rank == 0:  # Only the master process writes the file
        ...
        log_metrics(metrics_summary, output_file)

    cleanup_ddp()



# Multi-CPU Experiment Function

def run_multi_cpu_experiment(cpu_configs, train_dataset, batch_size, lr, num_epochs, output_file):
    results = []

    for num_cpus in cpu_configs:
        print(f"Running training with {num_cpus} CPUs")
        os.environ['OMP_NUM_THREADS'] = str(num_cpus)
        torch.set_num_threads(num_cpus)

        metrics_file = f"Metrics_{num_cpus}_cpus.json"

        torch.multiprocessing.spawn(
            train_ddp,
            args=(num_cpus, train_dataset, batch_size, lr, metrics_file),
            nprocs=num_cpus,
            join=True
        )
        ...
        with open(metrics_file, "r") as f:
            metrics = json.load(f)
            results.append({"cpus": num_cpus, "metrics": metrics})
...
if __name__ == "__main__":
    ...
    output_file = "multi_cpu_metrics.json"

    run_multi_cpu_experiment(cpu_configs, train_dataset, batch_size, lr, num_epochs, output_file)